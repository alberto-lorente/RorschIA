...
def classify_contents(article_text, model_Contents="svm_contents_V1-26-03.sav", tf_vectorizer="tfidf.pickle"):
    
    clf = pickle.load(open(model_Contents, "rb"))
    # global tf_vectorizer
    # global data
    tf_vectorizer = pickle.load(open(tf_vectorizer, "rb"))
    
    
    
    article= tf_vectorizer.transform([article_text])
    
    prediction = clf.predict(article)[0]
    
    return prediction

def classify_determinants(article_text, model_determinants="svm_determinants_V1-26-03.sav", tf_vectorizer="tfidf.pickle"):
    ...
    prediction = clf_determinants.predict(article)[0]
